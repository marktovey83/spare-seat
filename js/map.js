(function () {
  ...
  function draw(el, plus) {
    ...
    if (typeof L === "undefined") {
      const d = plus ? 0.35 : 0.12;
      el.innerHTML = '<iframe title="map" style="width:100%;height:100%;border:0;border-radius:16px" src="https://www.openstreetmap.org/export/embed.html?bbox=' +
        (h.lng - d) + "%2C" + (h.lat - d) + "%2C" + (h.lng + d) + "%2C" + (h.lat + d) +
        "&layer=mapnik&marker=" + h.lat + "%2C" + h.lng + '"></iframe>';
      return;
    }
  ...
  function inject() {
    ...
    if (!el) {
      ...
      el.id = "punter-map";
      const plus = who.indexOf("Plus") >= 0;
      el.className = plus ? "plus" : "free";
